...
export default function Home() {
  ...
  const fetchCompanies = async () => {
    try {
      ...
      console.error(e)
      setSnack('Failed to load companies')
    } finally {
    ...
    try {
      ...
      const poll = async () => {
        ...
        if (Date.now() - start < timeout) {
          ...
          setLoading(false)
          setSnack('Analysis started. Check the dashboard shortly.')
        }
      }
      poll()
    } catch (e) {
  ...
  <Container maxWidth="lg" sx={{ py: 4 }}>
    ...
    {/* MAIN LAYOUT - ONLY ONE container */}
              ...
              {companies.map((c, idx) => (
                <Grid item key={c.id} xs={12} sm={6} md={4}>
                  ...
                  >
                    <Card>
                      <CardActionArea
                        onClick={() => navigate(`/dashboard/${encodeURIComponent(c.id)}`)}
                      >
                        <CardContent>
              ...
              <Grid item xs={12}>
                <Stack direction="row" alignItems="center" spacing={2}>
                  ...
                  <Typography color="text.secondary">
                    Running analysis... this may take a couple of minutes.
                  </Typography>
                </Stack>
              </Grid>
            )}
